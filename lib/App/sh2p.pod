...
The variable is only used by the shell and not reliable in any other language.
...
As an extension to the POSIX (and Bourne) standard, Bash and Korn shells allow local
...
However there are issues with this.  Bourne/POSIX never supported this, so strictly the Bourne
...
in the Bourne syntax ('typedef var' has no effect), but supporting it in the Korn shell syntax only.
...
That behaviour could be converted, but would impose a considerable overhead.  Therefore
the environment block is not consulted.
...
String concatination is not always correct
